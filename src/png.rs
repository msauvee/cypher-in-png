...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(values: &[u8]) -> Result<Self> {
        if values[0..8] != Self::STANDARD_HEADER {
            return Err(Error::Other("Invalid header"));
        }
        let mut chunks: Vec<Chunk> = Vec::new();
        let mut index = 8;
        while index < values.len() {
            let length = Chunk::decode_length(&values[index..index+4])?;
            let offset = length + (3*4);
            chunks.push((&values[index..index + offset]).try_into()?);
            index = index + offset;
        }
